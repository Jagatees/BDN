...
## What This Is
AI-native vendor call delegation. User types a natural language request → Codex generates a job spec → Genspark dials the vendor via PSTN → Codex extracts a structured JSON outcome → report saved to Supabase per-user dashboard.

## Stack
- **Frontend:** Next.js 15 (App Router), TypeScript, Tailwind CSS
- **AI:** Codex Sonnet (`Codex-sonnet-4-6`) via `@anthropic-ai/sdk`
...
1. `POST /api/plan` → Codex converts natural language to `JobSpec`
2. `POST /api/call` → Genspark dials vendor, returns raw transcript
3. `POST /api/extract` → Codex cleans transcript + extracts `CallOutcomeSummary` → saved to Supabase

## Directory Structure
```
src/
├── app/
│   ├── api/plan/         Codex planning route
│   ├── api/call/         Genspark execution + mock fallback
│   ├── api/extract/      Codex extraction + Supabase save
